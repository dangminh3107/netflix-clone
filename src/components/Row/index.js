...
const rank = [
  {
    number: `<svg id="rank-1" width="100%" height="100%" viewBox="-20 0 70 154" class="svg-icon svg-icon-rank-1 top-10-rank">
                <path stroke="#595959" stroke-linejoin="square" stroke-width="4" d="M35.377 152H72V2.538L2 19.362v30.341l33.377-8.459V152z"></path>
            </svg>`
  },
  {
    number: `<svg id="rank-2" width="100%" height="100%" viewBox="0 0 80 154" class="svg-icon svg-icon-rank-2 top-10-rank">
                <path
                    stroke="#595959"
  ...
  {
    number: `<svg id="rank-3" width="100%" height="100%" viewBox="0 0 80 154" class="svg-icon svg-icon-rank-3 top-10-rank">
                <path
                    stroke="#595959"
                    stroke-linejoin="square"
                    stroke-width="4"
  ...
  {
    number: `<svg id="rank-4" width="100%" height="100%" viewBox="0 0 81 154" class="svg-icon svg-icon-rank-4 top-10-rank">
  ...
  {
    number: `<svg id="rank-5" width="100%" height="100%" viewBox="0 0 81 154" class="svg-icon svg-icon-rank-5 top-10-rank">
                <path
                    stroke="#595959"
                    stroke-linejoin="square"
                    stroke-width="4"
  ...
  {
    number: `<svg id="rank-6" width="100%" height="100%" viewBox="0 0 81 154" class="svg-icon svg-icon-rank-6 top-10-rank">
                <path
                    stroke="#595959"
                    stroke-linejoin="square"
                    stroke-width="4"
  ...
  {
    number: `<svg id="rank-7" viewBox="0 0 78 154" width="100%" height="100%" class="svg-icon svg-icon-rank-7 top-10-rank">
  ...
  {
    number: `<svg id="rank-8" width="100%" height="100%" viewBox="0 0 77 154" class="svg-icon svg-icon-rank-8 top-10-rank">
                <path
                    stroke="#595959"
                    stroke-linejoin="square"
                    stroke-width="4"
  ...
  {
    number: `<svg id="rank-9" viewBox="0 0 71 154" width="100%" height="100%" class="svg-icon svg-icon-rank-9 top-10-rank">
                <path
                    stroke="#595959"
                    stroke-width="4"
  ...
  {
    number: `<svg id="rank-10" width="100%" height="100%" viewBox="0 0 140 154" class="svg-icon svg-icon-rank-10 top-10-rank">
                <path
                    stroke="#595959"
                    stroke-linejoin="square"
                    stroke-width="4"
...
function Row({ index, title, fetchUrl, isLargeRow, movie, genre, onChange }) {
  const [genreOneRow, setGenreOneRow] = useState([])
  ...
  const checkGenre = (value) => {
    if (genre) {
      let listGenre = {id: value.id, genres: []};
      for (let i = 0; i < value.genre_ids.length; i++) {
        for (let j = 0; j < genre.length; j++) {
          if (value.genre_ids[i] === genre[j].id) {
            listGenre.genres.push(genre[j])
          }
        }
      }
      listGenre.genres = listGenre.genres.filter((v,i,a)=>a.findIndex(t=>(t.id===v.id))===i)
      return listGenre;
    }
  }
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < movies.length; i++) {
      list.push(checkGenre(movies[i]));
    }
    setGenreOneRow(list);
    return () => {};
  },[genre])

  const handleChange = (movie, genreOneRow) => {
    const gen = genreOneRow.filter(f => f.id === movie.id)
    onChange(movie, gen);
  }
  
  const handleSlideChange = (index) => {
    const prevArrow = document.querySelector(`.swiper[id='${index}'] .swiper-button-prev`)
    prevArrow.classList.add('active')
  }

  return (isLoading ? <Skeletons type='row'/> : (
    <div className="row">
      <h2>{title}</h2>
      <Swiper
        modules={[Navigation]}
        ...
        slidesPerView={movies?.length ? 'auto' : 1}
        spaceBetween={isLargeRow ? 0 : 10}
        id={index}
      ...
      >
          {movies.map((movie,index) => (
            ...
            >
              <div className="poster-img-contain">
                <img 
                  key={movie.id} 
                ...
                />
                <div className="poster-rank" dangerouslySetInnerHTML={{__html: isLargeRow ? rank[index].number : '' }}></div>
              </div>
              <div className="poster-control">
                <img 
                  src={`${baseUrl}${isLargeRow ? movie.backdrop_path : ''}`} 
                  alt={movie.name}
                  className="row_poster_img_largerow-backdrop"
                />
                <div className="poster-control-icon">
                  <PlayArrowIcon className='control' onClick={() => handleChange(movie, genreOneRow)}/>
                  <AddIcon className='control'/>
                  <ThumbUpOutlinedIcon className='control'/>
                  <ThumbDownOutlinedIcon className='control'/>
                  <ClearOutlinedIcon className='control'/>
                  <KeyboardArrowDownOutlinedIcon className='control'/>
                </div>
                <div className="poster-overview">
                  <span className="poster-movie-name">{movie.name || movie.original_title}</span>
                ...
                <div className="movies-genre">
                  {genreOneRow.length ? genreOneRow[index].genres.map((gen,id) => (
                    <span className="movie-genre" key={id}>{gen.name}</span>
                  )) : ''}
                </div>
              </div>
            </SwiperSlide>
